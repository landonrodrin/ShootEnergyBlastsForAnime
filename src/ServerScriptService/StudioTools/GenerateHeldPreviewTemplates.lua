...
local function copyAnimeGui(ResourcesFolder)
	local AnimeResources = ServerStorage.Modules:WaitForChild("Anime"):WaitForChild("Resources")
	local SourceGui = AnimeResources:FindFirstChild("AnimeGui")
	if not (SourceGui and SourceGui:IsA("BillboardGui")) then
		warn("Missing ServerStorage.Modules.Anime.Resources.AnimeGui; held preview UI resource was not generated.")
		return false
	end
